...
public class Notifier {
    ...
    public Notifier(Context context) {
        ...
        this.sharedPrefs = context.getSharedPreferences(Constants.SHARED_PREFERENCE_NAME, Context.MODE_MULTI_PROCESS);
        this.appSharedPrefs = context.getSharedPreferences("target", Context.MODE_MULTI_PROCESS);
        this.notificationManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
    }

    public void notify(String notificationId, String apiKey, String title, String message, String uri, String imgUrl) {
        Log.d(LOGTAG, "notify()...");
        ...
        Log.d(LOGTAG, "notificationImgUrl=" + imgUrl);
        ...
        if (isNotificationEnabled()) {
            ...
            intent.putExtra(Constants.NOTIFICATION_IMGURL, imgUrl);
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            ...
            for (RunningTaskInfo info : taskList) {
                Log.d(LOGTAG, "top:" + info.topActivity.getClassName());
                Log.d(LOGTAG, "base:" + info.baseActivity.getClassName());
                // 找到当前应用的task，并启动task的栈顶activity，达到程序切换到前台
                if (info.topActivity.getPackageName().equals("com.lltech.manager")) {
                    break;
                } else {
                    intent.setAction(Intent.ACTION_MAIN);
                    intent.addCategory(Intent.CATEGORY_LAUNCHER);
                    intent.setComponent(new ComponentName("com.lltech.manager", appSharedPrefs.getString("lastActivityName",
                            "com.lltech.manager.activity.SplashAty")));
                    break;
                }
            }
            ...
            // "notif://notification.adroidpn.org/")).append(apiKey).append(
            // "/").append(System.currentTimeMillis()).toString()));
            PendingIntent clearPendingIntent = PendingIntent.getBroadcast(context, 0, clearIntent, 0);
    ...
    public void setNotificationEnabled(boolean flag) {
        sharedPrefs.edit().putBoolean(Constants.SETTINGS_NOTIFICATION_ENABLED, flag).commit();
    }
