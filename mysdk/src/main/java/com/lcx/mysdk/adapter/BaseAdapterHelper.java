 ...
 * 		.setText(R.id.tvEmails, contact.getEmails().toString()).setText(R.id.tvNumbers, contact.getNumbers().toString()).getView();
 * </pre>
 */
public class BaseAdapterHelper {
     ...
     *            The image URL.
     * @param loadType
     *            The image load type Cons.ImageLoadType.
     * @return The BaseAdapterHelper for chaining.
     */
    public BaseAdapterHelper setImageUrl(int viewId, Object imageUrl, int loadType) {
        ImageView view = retrieveView(viewId);
        switch (loadType) {
            case Cons.ImageLoadType.VOLLEY:
//                ImageListener listener = ImageLoader.getImageListener(view, R.drawable.no_pic, R.drawable.no_pic);
...
//                imageLoader.get(imageUrl.toString(), listener);
                break;
            case Cons.ImageLoadType.PICASSO:
                if (imageUrl instanceof String) {
                    Picasso.with(context).load(imageUrl.toString()).config(Bitmap.Config.RGB_565).memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE).placeholder(R.drawable.no_pic).error(R.drawable.no_pic).into(view);
                } else if (imageUrl instanceof File) {
                    Picasso.with(context).load((File) imageUrl).config(Bitmap.Config.RGB_565).memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE).placeholder(R.drawable.no_pic).error(R.drawable.no_pic).into(view);
                } else if (imageUrl instanceof Integer) {
                    Picasso.with(context).load((int) imageUrl).config(Bitmap.Config.RGB_565).memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE).placeholder(R.drawable.no_pic).error(R.drawable.no_pic).into(view);
                }
                break;
        }
        return this;
