...
/**
 * 
  * @ClassName(类名)      : PacketWriter
  * @Description(描述)    : 修改：加入了心跳线程
  * @author(作者)         ：liuchunxu
  * @date (开发日期)      ：2015年6月19日 上午9:25:12
  *
 */
class PacketWriter {
	...
	public void startHeartBeatThread(long keepTime){
		HeartBeatThread thread = new HeartBeatThread();
		thread.setKeepTime(keepTime);
		thread.start();
	}
}
