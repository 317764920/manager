		...
		BaseAdapter
{
	...
	protected SparseArray<T> saved_datas = new SparseArray<T>();
	
	protected SparseBooleanArray checkbox = new SparseBooleanArray();
	...
	public BaseQuickAdapter(Context context, int layoutResId, List<T> data)
	{
		...
		this.saved_datas = new SparseArray<T>();
		for(int i = 0;i<data.size();i++){
			saved_datas.put(i, null);
		}
		this.checkbox = new SparseBooleanArray();
		for(int i = 0;i<data.size();i++){
			checkbox.put(i, false);
		}
	}
...
//		resetCheckBox();
	...
	public void setSaveDatas(int position, T obj){
		saved_datas.put(position, obj);
	}
	
	public T getSaveDatas(int position){
		return saved_datas.get(position);
	}
	
	public SparseArray<T> getSaveDatasObj(){
		return saved_datas;
	}
	
	public void setCheckBox(int position, Boolean b){
		checkbox.put(position, b);
	}
	
	public Boolean getCheckBox(int position){
		return checkbox.get(position);
	}
